...
export default async function CommandCentreDirection() {
  ...
  return (
    <div className="min-h-dvh bg-ink font-mono text-ivory-text">
      {/* -------------------------------------------------------- status bar */}
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-paper/15 px-5 py-2.5 text-2xs uppercase tracking-[0.14em]">
        <span className="text-ivory-text/50">
          Ajo Mercy · ops · {profile.full_name.split(" ")[0]}
        </span>
        <span className={totalWaiting > 0 ? "text-ochre" : "text-ivory-text/40"}>
      ...
      <div className="grid grid-cols-2 gap-px border-b border-paper/15 bg-paper/15 sm:grid-cols-3 lg:grid-cols-6">
        {[
          ...
          >
            ...
            <p className="mt-1 text-2xs uppercase tracking-[0.14em] text-ivory-text/45">{item.label}</p>
          </Link>
        ))}
      </div>
      ...
      {oldest && (
        <section className="border-b border-paper/15 px-5 py-7">
          <p className="text-2xs uppercase tracking-[0.16em] text-ivory-text/40">
          ...
          <div className="mt-3 flex flex-wrap items-end justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-xs text-ivory-text/50">
      ...
      <section className="px-5 py-6">
        <div className="flex items-baseline justify-between gap-4">
          <h2 className="text-2xs uppercase tracking-[0.16em] text-ivory-text/40">Queue</h2>
          <Link href="/admin/alajos" className="text-2xs uppercase tracking-[0.14em] text-ivory-text/50 hover:text-ivory-text">
            open full list
          </Link>
        </div>

        {queue.items.length === 0 ? (
          <p className="mt-6 text-sm text-ivory-text/40">Queue empty.</p>
        ) : (
          <table className="mt-4 w-full text-xs">
            <thead>
              <tr className="border-b border-paper/15 text-2xs uppercase tracking-[0.12em] text-ivory-text/40">
            ...
            <tbody>
              {queue.items.map((item, index) => (
                <tr key={item.id} className="border-b border-paper/10 hover:bg-paper/5">
                  <td className="py-2 text-ivory-text/30 tabular">{String(index + 1).padStart(2, "0")}</td>
                  <td className="py-2">
                    <Link href={`/admin/alajos/${item.id}`} className="text-ivory-text hover:text-ochre">
                      {item.business_name ?? "untitled"}
                    </Link>
                  </td>
                  <td className="py-2 text-ivory-text/50">{item.founder_name ?? "—"}</td>
                  <td className="py-2 text-ivory-text/50">{item.state ?? "—"}</td>
                  <td
                    className={`py-2 text-right tabular ${
                      item.completeness < 100 ? "text-ochre" : "text-ivory-text/50"
                    }`}
                  >
                    {item.completeness}%
                  </td>
                  <td className="py-2 text-right text-ivory-text/50">
                    {item.submitted_at ? formatRelative(item.submitted_at) : "—"}
                  </td>
                  <td className="py-2 text-right text-2xs uppercase tracking-[0.1em] text-ivory-text/40">
      ...
      <section className="border-t border-paper/15 px-5 py-6 pb-24">
        <h2 className="text-2xs uppercase tracking-[0.16em] text-ivory-text/40">Log</h2>
        <ol className="mt-3 space-y-1 text-xs">
          {activity.length === 0 ? (
            <li className="text-ivory-text/40">No entries.</li>
          ) : (
            activity.map((item) => (
              <li key={item.id} className="flex gap-3 text-ivory-text/60">
                <span className="shrink-0 text-ivory-text/30">
                  {new Date(item.createdAt).toLocaleTimeString("en-NG", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>
                <span className="text-ivory-text/80">{item.actor ?? "system"}</span>
